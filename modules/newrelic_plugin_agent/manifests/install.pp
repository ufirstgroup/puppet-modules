class newrelic_plugin_agent::install (
	...
	$pidfile = "/var/run/newrelic/${::newrelic_plugin_agent::proc_name}.pid"
	$confdir = "/etc/newrelic"
	$conffile = "${confdir}/newrelic_plugin_agent.cfg"

	$user  = $newrelic_plugin_agent::user
	$group = $newrelic_plugin_agent::group
	...
	file {
		...
		"${conffile}":
			...
			notify => Service["${newrelic_plugin_agent::proc_name}"];
		"/etc/init/${::newrelic_plugin_agent::proc_name}.conf":
			content => template('newrelic_plugin_agent/newrelic_plugin_agent_init_script.conf.erb');
		"/etc/init.d/${::newrelic_plugin_agent::proc_name}":
			ensure => 'link',
